...
const uri = `mongodb+srv://health-care-database:7Ia2uWEw1T416LBJ@cluster0.li11u.mongodb.net/?retryWrites=true&w=majority`;
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true });
async function run() {
    try {
        await client.connect();
        const serviceCollection = client.db("health-care-database").collection("services");
        const specialistCollection = client.db("health-care-database").collection("specialists");
        const userCollection = client.db("health-care-database").collection("users");
        const appointmentCollection = client.db("health-care-database").collection("appointments");
        ...
        app.get("/specialists", async (req, res) => {
            const specialists = await specialistCollection.find({}).toArray();
        ...
        app.get("/specialists/byTid/:id", async (req, res) => {
            try {
                const id = req.params.id;
    ...
    //     res.json(result);
    // })
    //     app.get("/specialists/byTid/:tid", async (req, res) => {
    ...
    // })
        ...
        app.delete("/appointments/:id", async (req, res) => {
            const id = req.params.id;
            const query = { _id: new ObjectId(id) };
            const deletedOrder = await appointmentCollection.deleteOne(query);
